...
const combineStrip = (starveSecond = false) => {
    const axisEnd = 20_000_000;
    return buildOperatorStrip(
        ...
        [
            ...
            named(
                ...
                starveSecond
                    ? exact(axisEnd - WINDOW - 2_000_000, [0])
                    : density(axisEnd, [2, 2, 2, 2]),
            ),
        ],
        // ⚠️ The output dies WITH the starved input under zip — it waits for
        // laggards — so a fixture that starves an input while leaving the
        // output live is not a state the runtime can produce.
        starveSecond
            ? exact(axisEnd - WINDOW - 2_000_000, [0])
            : density(axisEnd, [2, 2, 2, 2]),
        "out",
...
describe("trackModeFor", () => {
    it("forks a threshold and merges everything else", () => {
        expect(trackModeFor("threshold")).toBe("fork");
        expect(trackModeFor("combine")).toBe("merge");
        expect(trackModeFor("marker_merge")).toBe("merge");
    });
});

describe("layoutTrackDiagram — merge", () => {
    it("converges every input onto one output line", () => {
        const diagram = layoutTrackDiagram(combineStrip(), "merge", "data");
        const inputs = diagram.lanes.filter((lane) => lane.role === "input");
        const output = diagram.lanes.find((lane) => lane.role === "output")!;

        expect(inputs).toHaveLength(2);
        // The inputs sit at different heights; the output is on the centre line
        // BETWEEN them, which is what makes the shape read as a merge.
        expect(inputs[0].y).not.toBe(inputs[1].y);
        expect(output.y).toBeGreaterThan(inputs[0].y);
        expect(output.y).toBeLessThan(inputs[1].y);
        expect(output.y).toBe(diagram.hub.y);

        // One diagonal per input, all ending at the hub.
        expect(diagram.joins).toHaveLength(2);
        for (const join of diagram.joins) {
            expect(join.y2).toBe(diagram.hub.y);
        }
    });

    it("starts the output run where the inputs' runs end", () => {
        const diagram = layoutTrackDiagram(combineStrip(), "merge", "data");
        const input = diagram.lanes.find((lane) => lane.role === "input")!;
        const output = diagram.lanes.find((lane) => lane.role === "output")!;
        expect(output.x1).toBeGreaterThan(input.x2);
    });

    // ⚠️ Same acceptance test as the rows grammar, in the other shape: a
    // switchable view that cannot show the starved case would be a downgrade
    // dressed as a choice.
    it("marks the starved input's rail AND the output's", () => {
        const diagram = layoutTrackDiagram(combineStrip(true), "merge", "data");
        const [busy, starved] = diagram.lanes.filter(
            (lane) => lane.role === "input",
        );
        expect(busy.silent).toBe(false);
        expect(starved.silent).toBe(true);
        expect(starved.caption).toMatch(/^silent /);
        expect(diagram.lanes.find((lane) => lane.role === "output")!.silent).toBe(
            true,
        );
        // The rail carrying nothing is the one drawn as broken.
        expect(diagram.joins.filter((join) => join.silent)).toHaveLength(1);
    });

    it("places marbles along their own lane's run, not the card's full width", () => {
        const axisEnd = 20_000_000;
        const strip = buildOperatorStrip(
            "combine",
            {},
            [named("in1", exact(axisEnd, [0, 2_000_000]))],
            exact(axisEnd, [0]),
            "out",
            axisEnd,
        )!;
        const diagram = layoutTrackDiagram(strip, "merge", "data");
        const input = diagram.lanes.find((lane) => lane.role === "input")!;
        for (const marble of input.marbles) {
            expect(marble.x).toBeGreaterThanOrEqual(input.x1);
            expect(marble.x).toBeLessThanOrEqual(input.x2);
            expect(marble.y).toBe(input.y);
        }
    });
});

describe("layoutTrackDiagram — fork", () => {
    const thresholdStrip = () =>
        buildOperatorStrip(
            "threshold",
            { level: 0.5, direction: "either" },
            [named("in", density(10_000_000, [100, 100, 100, 100]))],
            exact(10_000_000, [0, 500_000]),
            "markers",
            10_000_000,
        )!;

    it("splits one input into an output branch and a passthrough branch", () => {
        const diagram = layoutTrackDiagram(thresholdStrip(), "fork", "markers");
        const input = diagram.lanes.find((lane) => lane.role === "input")!;
        const output = diagram.lanes.find((lane) => lane.role === "output")!;
        const passthrough = diagram.lanes.find(
            (lane) => lane.role === "passthrough",
        )!;

        // The input arrives on the centre line and the two branches straddle it.
        expect(input.y).toBe(diagram.hub.y);
        expect(output.y).toBeLessThan(input.y);
        expect(passthrough.y).toBeGreaterThan(input.y);
        expect(diagram.joins).toHaveLength(2);
    });

    it("colours the crossings branch as markers and the passthrough as data", () => {
        const diagram = layoutTrackDiagram(thresholdStrip(), "fork", "markers");
        expect(diagram.lanes.find((lane) => lane.role === "output")!.lane).toBe(
            "markers",
        );
        expect(
            diagram.lanes.find((lane) => lane.role === "passthrough")!.lane,
        ).toBe("data");
    });

    // ⚠️ The lower branch must NOT claim a count. A threshold's input is FRAMES
    // and its output is CROSSINGS, and one frame can produce several — so
    // "input.total - output.total" would be a fabricated number presented as a
    // measurement.
    it("gives the passthrough branch no label and no caption", () => {
        const diagram = layoutTrackDiagram(thresholdStrip(), "fork", "markers");
        const passthrough = diagram.lanes.find(
            (lane) => lane.role === "passthrough",
        )!;
        expect(passthrough.label).toBe("");
        expect(passthrough.caption).toBe("");
    });
