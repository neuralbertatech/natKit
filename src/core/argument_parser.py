...
def parse() -> argparse.Namespace:
    ...
    parser.add_argument('--com-port', help='The COM port to connect to for serial data',
                        type=str, metavar='COM')
    parser.add_argument('--channels', help='The number of channels connected',
                        type=int, metavar='Channels')

    connectParser = commandParser.add_parser('connect', help='Connect to a board to stream data')
    connectParser.add_argument('board', help='The boards to connect to',
                               choices=get_supported_boards())
    connectParser.add_argument('--output', help='Output stream to file',
                               type=pathlib.Path, metavar='File')

    simulateParser = commandParser.add_parser('simulate', help='Simulate data from a given board')
    simulateParser.add_argument('board', help='The boards to connect to',
                                choices=get_supported_boards())
    simulateParser.add_argument('--input', help='Input simulated data from file',
                                type=pathlib.Path, metavar='File')
