...
class KafkaManager:
    ...
    def create(host: str = None, port: str = None):
        ...
        return KafkaManager(host, port, broker, AdminClient({"bootstrap.servers": broker}))
    ...
    def create_messenger(self, topic_string: str):
        ...
        return Messenger(topic_name, self._create_data_consumer_conf(), self.producer_conf, encoder, schema)
    ...
    def _create_stream(self, meta_topic_string: str, data_topic_string: str):
        return Stream(meta_messenger=self.create_messenger(meta_topic_string), data_messenger=self.create_messenger(data_topic_string))

    def find_streams(self):
        topic_string_pairs = StreamHelper.find_topic_stream_pairs(self.query_topic_names())
        return [self._create_stream(topic_string_pair["meta"], topic_string_pair["data"]) for topic_string_pair in topic_string_pairs]
    ...
    def create_stream(self, name: str, data_encoder: Encoder, data_schema: Schema, meta_encoder: Encoder, meta_schema: Schema):
        id = random.randint(0, 2^64)
        data_topic_name = TopicName(id=id, type=TopicType.DATA, encoder_name=data_encoder.get_name(), schema_name=data_schema.get_name())
        meta_topic_name = TopicName(id=id, type=TopicType.META, encoder_name=meta_encoder.get_name(), schema_name=meta_schema.get_name())
        self.create_topic(data_topic_name.topic_string)
        self.create_topic(meta_topic_name.topic_string)
        return self._create_stream(meta_topic_name.topic_string, data_topic_name.topic_string)
